...
const AllUrls = () => {
    ...
    const handleSave = () => {
        if (formData) {
            ...
            axios
                ...
                .then(() => {
                    setUrls(
                        urls.map((url, index) => (index === editIndex ? (formData as UrlType) : url))
                    );
                    setEditIndex(null);
                    setFormData(null);
                    setOldShortUrl("");
    ...
    const confirmDelete = () => {
        if (deleteIndex !== null) {
            ...
            axios
                .delete(`${api}/url/delete/${url.shortUrl}`, {
                    headers: {
                        "Content-Type": "application/json",
                        authorization: Cookies.get("accessToken"),
                    },
                                    ...
                                    {editIndex === index ? (
                                        <div className="space-y-4 w-full">
                                            ...
                                            <div className="flex items-center space-x-2 w-full">
                                                <label
                                                    className="text-sm w-24 text-gray-400"
                                                    htmlFor="shortUrl"
                                                >
                                                    Short URL:
                                                </label>
                                            ...
                                            <div className="flex w-full">
                                                ...
                                                />
                                                <button
                                                    className="ml-2 px-3 py-[2px] h-full bg-yellow text-black rounded hover:bg-amber-500 transition-colors duration-200"
                                                    onClick={clearDate}
                                                >
                                                    Clear Date
                                                </button>
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                Click on{" "}
                                                <span className="font-black">the clear button</span> to
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                Type <span className="font-black">&lt;null&gt;</span> to
                                                reset the password
                                            </p>
                                            <div className="flex w-full">
                                                <label
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                Type <span className="font-black">-1</span> to remove
                                                the max clicks
                                            </p>
                                            <p className="text-xs text-gray-500 mt-1">
                                                Leave untouched the fields you don&apos;t want to change
                                            </p>
                                            <button
                                        ...
                                        <>
                                            ...
                                            {url.expirationDate && (
                                                <p className="text-sm text-gray-400">
                                                    Expiration Date: {url.expirationDate}
                                                </p>
                                            )}
                                            <p className="text-sm text-gray-400">
                                                Expired: {url.expired.toString()}
                                            </p>
